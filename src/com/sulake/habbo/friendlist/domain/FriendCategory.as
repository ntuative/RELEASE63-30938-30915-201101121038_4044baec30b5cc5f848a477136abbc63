package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_881 >= this.getPageCount())
         {
            this.var_881 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_210.length / const_324);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_881 * const_324;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_881 + 1) * const_324,this.var_210.length);
      }
