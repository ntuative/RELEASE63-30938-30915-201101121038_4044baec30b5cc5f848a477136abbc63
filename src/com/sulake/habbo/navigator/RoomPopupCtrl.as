package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      private var var_2139:RoomThumbnailRenderer;
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2139 = new RoomThumbnailRenderer(param1);
         this.var_1171 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_169 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_169.visible = true;
         Util.hideChildren(this.var_169);
         this.refreshRoomName(this.var_169,this.var_108);
         if(this.var_108.ownerName != "" && this.var_108.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_169,!!this.var_108.event ? this.getEventCtg(this.var_108.categoryId) : this.getRoomCtg(this.var_108.categoryId));
         this.refreshRoomDesc(this.var_169,this.var_108);
         this.refreshExtraCont();
         this.refreshInfo(this.var_169,"trading_allowed",this.var_108.allowTrading);
         this.refreshInfo(this.var_169,"doormode_doorbell",this.var_108.doorMode == 1);
         this.refreshInfo(this.var_169,"doormode_password",this.var_108.doorMode == 2);
         Util.moveChildrenToColumn(this.var_169,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_169.height = Util.getLowestPoint(this.var_169);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_169,this.var_108.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_169.findChildByName("roomowner"));
         if(this.var_108.ownerName != "" && this.var_108.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_169.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1171.refreshTags(_loc1_,this.var_108.tags);
         this.refreshTextWithCaption("rating",_loc1_,"" + this.var_108.score);
         if(this.var_108.event)
         {
            this.refreshTextWithCaption("startedat",_loc1_,"" + this.var_108.eventCreationTime);
         }
         this.var_2139.refreshThumbnail(_loc1_,this.var_108.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_256,Tabs.const_209,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
