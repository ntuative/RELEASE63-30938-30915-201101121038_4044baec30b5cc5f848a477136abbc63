package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= this.var_474.numGridItems)
         {
            return;
         }
         this.select(this.var_474.getGridItemAt(param1) as IWindowContainer);
      }
      
      private function populate(param1:IHabboWindowManager) : void
      {
         if(this._view == null || this.var_474 == null)
         {
            return;
         }
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         this.var_474.destroyGridItems();
         this.var_310 = null;
         for each(_loc2_ in this.colors)
         {
            _loc3_ = param1.buildFromXML(this.var_1705) as IWindowContainer;
            ...
            _loc3_.width = this.var_587.width;
            _loc3_.height = this.var_587.height;
            this.var_474.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(this.var_587.width,this.var_587.height,true,0);
               _loc4_.bitmap.copyPixels(this.var_587,this.var_587.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(this.var_1095.width,this.var_1095.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(this.var_893.width,this.var_893.height,true,16777215);
               _loc6_.bitmap.copyPixels(this.var_893,this.var_893.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = this.var_474.getGridItemIndex(param1.target as IWindow);
      ...
      private function storeAssets(param1:IAssetLibrary) : void
      {
         ...
         this.var_1705 = XML(_loc2_.content);
         _loc3_ = BitmapDataAsset(param1.getAssetByName("dimmer_color_frame"));
         this.var_587 = BitmapData(_loc3_.content);
         ...
         this.var_893 = BitmapData(_loc3_.content);
      }
