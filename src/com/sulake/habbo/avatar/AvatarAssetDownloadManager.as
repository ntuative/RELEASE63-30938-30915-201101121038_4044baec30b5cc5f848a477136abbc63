package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_1869:int = 100;
      
      private const const_1358:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_802 = new Timer(this.const_1869,1);
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_364)
         {
            this.var_364.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_364.addEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_364)
         {
            this.var_364.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_364.removeEventListener(AssetLoaderEvent.const_41,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_801);
            if(this.var_1218.indexOf("?") > 0)
            {
               _loc2_ = this.var_1218 + "&retry=" + this.var_801;
            }
            else
            {
               _loc2_ = this.var_1218 + "?retry=" + this.var_801;
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1482 = true;
         for each(_loc2_ in this.var_976)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2018,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_977[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_977[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1482)
         {
            this.var_976.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_800[_loc3_];
         }
         while(_loc6_ < this.var_647.length)
         {
            _loc5_ = this.var_647[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_647.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_802.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_646.length > 0 && this.var_647.length < this.const_1358)
         {
            _loc1_ = this.var_646[0];
            _loc1_.startDownloading();
            this.var_647.push(this.var_646.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_646.indexOf(param1) == -1 && this.var_647.indexOf(param1) == -1)
         {
            this.var_646.push(param1);
            this.processPending();
         }
      }
