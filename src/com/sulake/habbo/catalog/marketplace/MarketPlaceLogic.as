package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      private static const const_1168:String = "poster";
       
      
      public const const_1665:int = 1;
      
      public const const_1660:int = 2;
      
      public const const_1919:int = 3;
      ...
      private var var_2008:MarketplaceItemStats;
      ...
      public function dispose() : void
      {
         ...
         if(this.var_154)
         {
            for each(_loc1_ in this.var_154)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
         ...
         if(this.var_194)
         {
            for each(_loc1_ in this.var_194)
            {
               if(_loc1_ != null)
               {
                  _loc1_.dispose();
      ...
      {
         this.requestOffers(this.var_2003,this.var_2005,this.var_2001,this.var_2002);
      }
      ...
      {
         if(!this.var_154 || !this._catalog || !this._catalog.getPurse())
         {
            return;
         }
         var _loc2_:MarketPlaceOfferData = this.var_154.getValue(param1) as MarketPlaceOfferData;
         if(!_loc2_)
         {
            return;
         }
         ...
         this.showConfirmation(this.const_1665,_loc2_);
      }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = this.var_194.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = this.var_194.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_403)
            {
               this.var_194.remove(_loc2_);
               _loc3_.dispose();
            }
         }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_154)
         {
            for each(_loc5_ in this.var_154)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            this.var_154.add(_loc4_.offerId,_loc6_);
         }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         if(this.var_194)
         {
            for each(_loc5_ in this.var_194)
            {
               if(_loc5_ != null)
               {
                  _loc5_.dispose();
         ...
         this.var_2007 = _loc3_.creditsWaiting;
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            _loc6_.timeLeftMinutes = _loc4_.timeLeftMinutes;
            this.var_194.add(_loc4_.offerId,_loc6_);
         }
         if(this._visualization == null)
         {
            return;
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         {
            item = this.var_154.remove(parser.requestedOfferId);
            if(item != null)
            {
               item.dispose();
            }
         ...
         {
            updateItem = this.var_154.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               --updateItem.offerCount;
               this.var_154.add(parser.offerId,updateItem);
            }
            this.var_154.remove(parser.requestedOfferId);
            this.showConfirmation(this.const_1660,updateItem);
            this._visualization.listUpdatedNotify();
         }
      ...
      public function onCancelResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            item = this.var_194.remove(parser.offerId);
            if(item != null)
            {
               item.dispose();
            }
      ...
      {
         if(param1.furniCategoryId != this.var_2004 || param1.furniTypeId != this.var_2006)
         {
            return;
         }
      ...
      private function isPosterItem(param1:IMarketPlaceOfferData) : Boolean
      {
         ...
         if(param1.furniType == 2 && param1.stuffData != null)
         {
            ...
            {
               _loc4_ = _loc3_.name;
               if(_loc4_ != null && _loc4_ == const_1168)
               {
                  _loc2_ = true;
               }
            }
