package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization
   {
      ...
      private const const_1608:int = 1;
      
      private const const_1609:int = 2;
      
      private const const_1887:int = 40;
      
      private const const_1886:int = 10;
      
      private var var_702:Array;
      
      private var var_1316:MarketPlaceOfferData;
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_457)
         {
            this.var_457.removeEventListener(TimerEvent.TIMER,this.onPopulationTimer);
            this.var_457 = null;
         }
      }
      ...
      private function selectSearchCategory(param1:String) : void
      {
         ...
         switch(param1)
         {
            ...
            case "search_by_activity":
               _loc5_ = "marketplace_search_simple";
               this.var_702 = [3,4,5,6];
               break;
            case "search_advanced":
               _loc5_ = "marketplace_search_advanced";
               this.var_702 = [1,2,3,4,5,6];
               break;
            default:
               return;
         }
         ...
         if(_loc7_ != null)
         {
            _loc7_.populate(this.getSortKeys(this.var_702));
            _loc7_.selection = 0;
         }
      }
      ...
      private function updateList() : void
      {
         ...
         this.updateStatusDisplay(this.const_1609,_loc2_.length);
         if(!this.var_457)
         {
            this.var_457 = new Timer(25);
            this.var_457.addEventListener(TimerEvent.TIMER,this.onPopulationTimer);
         }
         this.var_1073 = 0;
         this.populateList();
         this.var_457.start();
      ...
      private function populateList() : Boolean
      {
         ...
         while(_loc1_ < 5)
         {
            if(this.var_1073 >= this._offers.length)
            {
               return true;
            }
            this.addListItem(this._offers.getWithIndex(this.var_1073));
            ++this.var_1073;
            _loc1_++;
         }
         return false;
      ...
      {
         var _loc5_:* = null;
         this.updateStatusDisplay(this.const_1608);
         var _loc1_:int = -1;
         var _loc2_:int = -1;
         var _loc3_:String = "";
         var _loc4_:int = 1;
         ...
         if(_loc6_ && _loc6_.selection >= 0 && _loc6_.selection < this.var_702.length)
         {
            _loc4_ = this.var_702[_loc6_.selection];
         }
         ...
         else if(param1.type == WindowEvent.const_106)
         {
            ...
            switch(_loc7_.name)
            {
               ...
               case "max_price_input":
                  _loc8_ = this.const_1886;
                  break;
               case "search_input":
                  _loc8_ = this.const_1887;
                  break;
               default:
                  return;
            }
         ...
         if(param1.type == WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK)
         {
            switch(param2.name)
            {
               case "back_button":
                  ...
                  break;
               case "buy_button":
                  this.marketPlace.buyOffer(this.var_1316.offerId);
            }
            return;
         }
         if(param1.type == WindowEvent.const_54)
